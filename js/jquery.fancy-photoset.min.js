...
f.append(b("<ul/>").attr("id",e).addClass("fancyPhotoset"));return b.getJSON(a,function photo(e){b.each(e.photoset.photo,function(e,a){var d,h;h=b("<img/>").attr({src:g.prototype.urlFor(a,{size:c.small})});d=b("<a/>").attr({href:g.prototype.urlFor(a,{size:c.large}),rel:"flickr-"+c.photosetId,});d.html(h).fancybox(c.fancybox);/*c.captions&&h.after(b("<span/>").addClass("caption").text(a.title));*/return f.find("ul").append(b("<li/>").html(d))});if(c.firstOnly)return f.find("li").not(":first").hide()})};
